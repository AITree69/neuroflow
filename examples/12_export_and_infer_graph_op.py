"""Stage 2 Sprint 3.2 demo (2/2): C++ inference of the exported GraphOp
NeuroIR.

Mirrors examples/10 for the GraphOp (GCN-style) operator. The exported
JSON is loaded for the PyTorch reference + pure-Python roundtrip; the
binary is loaded into the C++ runtime for the parity check + latency
benchmark.

Run:
    python examples/12_export_and_infer_graph_op.py
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 2 GraphOp inference + benchmark")
    parser.add_argument("--ir", type=str, default="./artifacts/graphop_demo.neuroir")
    parser.add_argument("--bin", type=str, default="./artifacts/graphop_demo.nneuroir")
    ...
    if not ir_path.exists():
        raise FileNotFoundError(
            f"{ir_path} not found — run 11_train_graph_op.py first."
        )
    ...
    val_set = TokenMixerDemo1dDataset(
        n_samples=1, cfg=TokenMixerDemo1dConfig(n_points=args.n_nodes, seed=99), seed=99
    )
    x, _ = val_set[0]
    ...
    if not bin_path.exists():
        ...
        print(
            f"   C++ latency (batch=1, n_nodes={args.n_nodes}, "
            f"hidden={spec.config['hidden_dim']}) = {cpp_latency_ms:.3f} ms"
        )
    ...
    print(
        f"   Python latency (batch=1, n_nodes={args.n_nodes}, "
        f"hidden={spec.config['hidden_dim']}) = {py_latency_ms:.3f} ms"
    )
