"""Stage 2 Sprint 1 demo (2/2): C++ inference of the exported FNO2d NeuroIR.

Mirrors examples/02_export_and_infer.py for the 2D operator.

Run:
    python examples/04_export_and_infer_fno2d.py
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 2 FNO2d inference + benchmark")
    parser.add_argument("--ir", type=str, default="./artifacts/fno2d_poisson.neuroir")
    parser.add_argument("--bin", type=str, default="./artifacts/fno2d_poisson.nneuroir")
    ...
    if not ir_path.exists():
        raise FileNotFoundError(
            f"{ir_path} not found — run 03_train_fno2d.py first."
        )
    ...
    x_np = rng.standard_normal((1, args.h, args.w, 1)).astype(np.float32)
    ...
    if not bin_path.exists():
        ...
        try:
            ...
            tmp_in = Path("./artifacts/_tmp_in_fno2d.npy")
            tmp_out = Path("./artifacts/_tmp_out_fno2d.npy")
            np.save(tmp_in, x_np)
            ...
            print(
                f"   C++ latency (batch=1, {args.h}x{args.w}) = {cpp_latency_ms:.3f} ms"
            )
    ...
    print(f"   Python latency (batch=1, {args.h}x{args.w}) = {py_latency_ms:.3f} ms")
    ...
    if y_cpp is not None:
        ...
        title = f"FNO2d: PyTorch vs C++ (max abs diff = {diff_cpp:.2e})"
        cmp_pred = y_cpp[0, ..., 0]
    else:
        title = "FNO2d: PyTorch vs Python-IR (roundtrip)"
        cmp_pred = y_py[0, ..., 0]
    plot_2d_field_comparison(
        y_pt[0, ..., 0],
        cmp_pred,
        title=title,
        save_path="./artifacts/fno2d_poisson_roundtrip.png",
    )
    print("==> Done.")
